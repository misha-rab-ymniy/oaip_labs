...
{
     int a=0;
     QPoint *p=new QPoint [top*2+1]; //Массив для хранения координат вершин звезды
...
double Star5::square(){
    return this->getK() * this->getK() * (M_PI * radius * radius + (M_PI * Radius * Radius - M_PI * radius * radius) / 3);
}

double Star5::perimetr(){
    return 10 * size * getK();
}
